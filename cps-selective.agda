-- Formalization of λC & CPS translation of fine-grained λF

module CPS-Selective where

open import LambdaF-FG hiding (〚_〛τ; 〚_〛μ; cpsv; cpsp; cpsi; append; cons; kid)
...
cpsp : {var : CTy → Set} {τ : Ty} →
       PExp[ var ∘ 〚_〛τ  ] τ → CExp[ var ] 〚 τ 〛τ
        
cpsi : {var : CTy → Set} {τ α β : Ty} {μα μβ : Tr} →
       IExp[ var ∘ 〚_〛τ ] τ ⟨ μα ⟩ α ⟨ μβ ⟩ β →
...
cpsv (Bol b) = Bol b
cpsv (Str s) = Str s
cpsv {var = var} (PAbs f) = Abs (λ x → cpsp (f x))
cpsv {var = var} (IAbs f) = Abs (λ x → Val (cpsi (f x)))

cpsp (Val v) = Val (cpsv v)

cpsp (PApp e₁ e₂) = App (cpsp e₁) (cpsp e₂)
        
cpsp (PPlus e₁ e₂) = Plus (cpsp e₁) (cpsp e₂)
...
cpsi (Exp e) =
  Abs (λ k → Val (Abs (λ t →
    App (App (Val (Var k)) (cpsp e)) (Val (Var t)))))
